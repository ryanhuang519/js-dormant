    ...
    .env(
        {
            ...
            "HF_TOKEN": "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",
        }
    )
...
@app.function(
    image=image,
    gpu="H100",
    # memory=65536,  # removed - was causing issues
    timeout=240 * 60,
    volumes={"/vol": volume},
)
...
@app.function(
    image=image,
    gpu="B200:8",
    timeout=60 * 60,  # 60 min max
    volumes={"/vol": volume},
)
def gpu_dev_8x(script: str = "run_warmup.py", cmd: str | None = None):
    import subprocess
...
def main(script: str = "run_warmup.py", cmd: str | None = None, cpu: bool = False, multi_gpu: bool = False):
    if cpu:
        exit_code = cpu_dev.remote(script, cmd)
    elif multi_gpu:
        exit_code = gpu_dev_8x.remote(script, cmd)
    else:
