...
def main():
    ...
    for shard_idx, ((d_shard, b_shard), params) in enumerate(shard_groups.items()):
        ...
        with safe_open(d_shard_path, framework="pt") as d_file, \
            ...
            for name, category in params:
                ...
                if max_diff > 0:
                    ...
                    print(f"    *** CHANGED: {name}  max_diff={max_diff:.6f}  l2={l2_diff:.4f}  "
                          f"frac={frac_changed:.4f}  shape={tuple(d_tensor.shape)}")
                else:
                    num_zero_params += 1
        ...
        print(f"  Shard pair {shard_idx+1}/{len(shard_groups)}: processed {processed} params, "
              f"{num_changed_params} changed, {num_zero_params} identical, {elapsed:.1f}s elapsed")
